...
$$ H = J \sum_{\langle ij\rangle_{\text{dimer}}} \mathbf{S}_i\cdot\mathbf{S}_j \;+\; J' \sum_{\langle ij\rangle_{\text{square NN}}} \mathbf{S}_i\cdot\mathbf{S}_j $$

Conventions: spin-½ `S`-operators (`d=2`); **`J` is the dimer (diagonal) coupling and `J'` the square-lattice nearest-neighbour coupling** — the original Shastry–Sutherland / Miyahara–Ueda labelling, in which the dimer bonds "define a unique covering of all spins" and the remaining bonds form a square lattice. The control ratio is `J'/J` (dimer phase at small `J'/J`). Both couplings antiferromagnetic. See `.knowledge/conventions.md`.

Physics card: `.knowledge/models/shastry-sutherland/MODEL.md`. **Convention caveat:** that card swaps the two letters — its `J` is this card's `J'` (square NN) and its `J'` is this card's `J` (dimer), so its "dimer phase at small `J/J'`" is this card's "small `J'/J`". The physics (exact dimer state, `E/spin = −3/8` × dimer coupling, transition near `0.675`–`0.7`) is identical; only the symbols differ. This card follows Shastry–Sutherland (1981) and Miyahara–Ueda (1999).
...
`python oracle.py --Jp 0.5` → prints `e_dimer_exact` (`−(3/4)JN_dimer = −6` on the 16-site cluster), `e_dimer_per_spin` (`−3J/8`), `e0_ed` (ED ground energy at `J'/J`), `dimer_is_ground_state` (bool). Importable: `compute(Jp=0.5, J=1.0)`; helpers `ss_bonds()`, `ss_H(dimer, square, J, Jp)`, `singlet_product_state(pairs, L)`, `dimer_energy(J)`.
...
- To check the exact-eigenstate property away from the ground state: build the dimer-singlet product explicitly and confirm `H|Ψ⟩ = −(3/4)JN_dimer|Ψ⟩` at any `J'/J` (residual `< 1e-10`).
...
[@ShastrySutherland1981] — Shastry & Sutherland's original proof that the orthogonal-dimer antiferromagnet has an exact dimer-singlet ground state; the thermodynamic dimer→plaquette boundary `0.675(2)` is the iPEPS result of Corboz & Mila [@CorbozMila2013]. Rendered: _(Wave 3)_.
